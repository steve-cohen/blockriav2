...
function renderPortfolio(clientId, currentPortfolioId, portfolios) {
	const portfolioName = portfolios.filter(({ portfolioId }) => portfolioId.S === currentPortfolioId)[0].portfolioName.S
	...
	return (
		<>
			...
			<Link to={`/advisor/portfolios/assign?portfolioId=${currentPortfolioId}&clientId=${clientId}`}>change</Link>)
		</>
	)
}
...
function renderPortfolioAssign(clientName, clientId) {
	return (
		<>
			...
			<Link className='AssignPortfolio' to={`/advisor/portfolios/assign?clientName=${clientName}&clientId=${clientId}`}>
				Assign a Portfolio
			</Link>
		</>
	)
	...
	function renderClient({ clientId, clientName, nonzeroAccounts, portfolioId }) {
		...
		return (
			<tr key={clientId.S}>
				...
				<td>
					{portfolioId.S
						? renderPortfolio(clientId.S, portfolioId.S, portfolios)
